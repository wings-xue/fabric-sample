...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    # 基于crypto-config-org1.yaml创建身份信息
    set -x
  ...
  # 产生sdk配置文件
  # 具体sdk如何配置
  # 参考https://github.com/hyperledger/fabric-sdk-go
...
function networkUp() {
  ...
  # artifacts这里的artifacts好像只是创世区块
  if [ ! -d "organizations/peerOrganizations" ]; then
    createOrgs
    # 产生创世区块，产生channel配置文件在后面的createChannelTx
    createConsortium
  fi
